...
int main(void) {
  ...
  while(1) {
    ...
    // input
    u16 input = ~KEYINPUT;
    if (input & KEY_RIGHT) x_loc++;
    if (input & KEY_LEFT)  x_loc--;
    if (input & KEY_DOWN)  y_loc--;
    if (input & KEY_UP)    y_loc++;

    if (x_loc > SCREEN_WIDTH - BALL_WIDTH) {
      x_loc = SCREEN_WIDTH - BALL_WIDTH;
    } else if (x_loc < 0) {
      x_loc = 0;
    }
    if (y_loc > SCREEN_HEIGHT - BALL_HEIGHT) {
      y_loc = SCREEN_HEIGHT - BALL_HEIGHT;
    } else if (y_loc < 0) {
      y_loc = 0;
    }
